...
/* ---------------------------------------------------------------
   Mock AI import — replace with a real call to your AI service
   module (job parser) once the backend exists.
---------------------------------------------------------------- */
...
export default function PostJobPage() {
  ...
  const addSkill = () => {
    ...
    if (value && !job.skills.includes(value)) {
      setJob((j) => ({ ...j, skills: [...j.skills, value] }));
    }
    setSkillInput("");
  };
    ...
    <main className="min-h-screen bg-white">
      ...
      <header className="border-b border-border bg-[#134544]">
        <nav className="mx-auto flex max-w-3xl items-center justify-between px-6 py-4">
          ...
          <a href="/employers" className="text-sm font-medium text-muted hover:text-white/70 text-white">Save & exit</a>
        </nav>
      </header>
            ...
            <motion.div key="choose" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.25 }}>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                <button
                  ...
                  className="flex flex-col items-start gap-3 rounded-xl border border-border p-5 text-left transition-colors hover:border-brand hover:bg-brand-light/50"
                >
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-relaxed text-muted">
                      Paste a job description from anywhere and let Baseman structure it for you.
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-relaxed text-muted">
                      Prefer to type it yourself? Fill out the listing fields directly.
                    </p>
                  </div>
                </button>
              </div>
          ...
          {method === "ai" && importState !== "imported" && (
            <motion.div key="ai-paste" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.25 }}>
              ...
              <button
                ...
                disabled={!pastedText.trim() || importState === "importing"}
                className="mt-3 inline-flex items-center gap-2 rounded-lg bg-brand px-5 py-2.5 text-sm font-medium text-white transition-opacity hover:bg-brand/90 disabled:cursor-not-allowed disabled:opacity-40"
              >
                {importState === "importing" ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" /> Structuring with AI…
                  </>
                ) : (
                  <>
            ...
            <motion.div key="form" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.25 }}>
              ...
              <div className="mt-4 flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-ink">
                  {method === "ai" ? "Structured — review before publishing" : "Job details"}
                </p>
              </div>
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="mt-1.5 flex flex-wrap items-center gap-2 rounded-lg border border-border p-3">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      />
                      <button onClick={addSkill} aria-label="Add skill" className="text-muted hover:text-brand">
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="mt-1.5 w-full resize-none rounded-lg border border-border px-3.5 py-2.5 text-sm text-ink placeholder:text-muted focus:outline-none focus:ring-1 focus:ring-brand"
                  />
                </div>
              </div>

              {/* ---------------- Application settings: AI auto-reject ---------------- */}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-relaxed text-muted">
                        Applicants scoring below your threshold are marked as not selected
                        automatically, so they don't sit in your queue indefinitely.
                      </p>
                    </div>
                  </div>
                  ...
                  >
                    <span
                      className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform ${
                        autoReject ? "translate-x-[22px]" : "translate-x-0.5"
                      }`}
                    />
                  </button>
                </div>
                      ...
                      <div className="mt-5 border-t border-border pt-5">
                        ...
                        <div className="mt-4 flex gap-2.5 rounded-lg bg-brand-light p-3">
                          ...
                          <p className="text-xs leading-relaxed text-ink">
                            Applicants below {threshold}% are notified automatically that they
                            weren't selected — no one is left waiting on a listing they'll
                            never hear back on. You can still view every applicant regardless
                            of this setting.
                          </p>
                        </div>
                      </div>
